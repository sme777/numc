...
### Samson
> Samson implemented the following functions in Task 1
###### allocate_matrix
...
###### allocate_matrix_ref
``allocate_matrix_ref`` was the last function we worte in this part 1. Also, it was the primary cause of pain and agony in the debugging. Initially, we thought that the differences between ``allocate_matrix`` and this function were the offsets and the exisiting matrix. We did not realize that the underlying structure need be preserved in both matricies. we fixed this in the debugging process of the Task 3, and although in the end it seemed quite straightforward, the pointer arithmetic was quite difficult at first. 
###### deallocate_matrix
...
### Michael
Michael helped Samson write the functions in a pair programming format. Michael also helped Samson in the debugging process and took an active role in every step of the process.

## Task 2:  Writing the setup file
### Michael
Michael wrote the setup file.
### Samson
Samson worked with Michael, and helped him find resources when neccessary.

## Task 3: Writing the Python-C interface
### Samson
> Samson implemented the following functions in Task 3
#### Number Methods
...
###### Matrix61c_add
It took us some time to understand how the Python-C interface would work and the correct way to implement this function. Essentially, it was calling the function we already wrote in Task 1, with the caveat that we needed to unpack arguments and do error checking. After reading the Python docs on the C interface, we implemented this function with ease.
###### Matrix61c_sub
Matrix61c_sub was much like ``Matrix61c_add``, we still needed to do type casting of python objects and also error checking, after which we just called the matrix_sub function we already implemented in Task 1.
###### Matrix61c_multiply
...
### Michael
Michael worked closely with the Samson, as he filled each function step by step. Michael and Samson worked in a pair programming format, and Michael often answered questions Samson had and debugged with him some tedious functions. Michael's input in Task 3 was an invaluable part of making everything actually work. 
## Task 4: Speeding up matrix operations

### Michael
> Michael improved the following functions in Task 4
###### allocate_matrix
###### allocate_matrix_ref
###### deallocate_matrix
###### get
